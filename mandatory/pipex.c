...
/*   Updated: 2025/01/30 04:05:35 by ael-gady         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	The_dutiful_child(char **av, char **envp, t_pipex *p)
{
	...
	if (dup2(input_fd, STDIN_FILENO) == -1)
	{
		...
		close(p->pipe_fd[0]);
		close(p->pipe_fd[1]);
		ft_error("Error: dup2(input_fd, STDIN_FILENO)");
	}
	if (dup2(p->pipe_fd[1],  STDOUT_FILENO) == -1)
	{
		...
		close(p->pipe_fd[0]);
		close(p->pipe_fd[1]);
		ft_error("Error: dup2(pipe_fd[1], STDOUT_FILENO)");
	}
	close(input_fd);
	close(p->pipe_fd[0]);
	close(p->pipe_fd[1]);
...
void	al_maari(char **av, char **envp, t_pipex *p)
{
	...
	if (dup2(output_fd, STDOUT_FILENO) == -1)
	{
		...
		close(p->pipe_fd[0]);
		close(p->pipe_fd[1]);
		ft_error("Error: dup2(output_fd, STDOUT_FILENO)");
	}
	if (dup2(p->pipe_fd[0], STDIN_FILENO) == -1)
	{
		...
		close(p->pipe_fd[0]);
		close(p->pipe_fd[1]);
		ft_error("Error: dup2(pipe_fd[0], STDIN_FILENO)");
	}
	close(output_fd);
	close(p->pipe_fd[0]);
	close(p->pipe_fd[1]);
